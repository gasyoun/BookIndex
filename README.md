...
Текущая редакторская очередь: 23 suspicious heads (23 помечены `needs_review`, 0 без triage), 22 sort inversions, 1 duplicate-head group, 8 cross-book duplicate candidates. Из manual audit найдено 11 из 11 терминов; без точного `head` сейчас не остаётся терминов.
...
4. Начать рост контекстного покрытия с `lexicon` и `subject_index`.
5. Уточнить правило для `lexicon_reverse`: наследовать контексты из `lexicon` или считать reverse-list отдельной индексной проекцией.
6. Проверить 8 cross-book duplicate candidates через quality queue.

...
В режиме `корпус` dropdown поиска группирует результаты по источнику и показывает понятное empty state, если совпадений нет. Списки сущностей, карточки, KWIC-строки, Markdown exports карточек/разделов, `page_trends` exports и VIZ header также показывают текущую книгу/source context, а Markdown exports дополнительно включают `book_id`, чтобы будущая multi-book навигация не требовала нового UI-паттерна.

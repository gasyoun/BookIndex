...
- `tests/context-entry-pack.json` и readable checklist `tests/context-entry-pack.md` генерируются через `npm run content:audit` как первый 25-item manual context-entry pack из `context_priority_top`; они не правят content data.

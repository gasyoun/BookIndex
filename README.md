...
- **Заложена корпусная оболочка UI**: `app_data.json` содержит explicit `APP_DATA.corpus` с текущей книгой как первым источником, runtime принимает aliases вида `#v4/books/<book_id>/...` и резервирует тип `video_catalog` для будущих 200 видео с тайм-кодами и стенограммами.
...
Текущий интерфейс остаётся совместимым с `#v4/...`, а `app_data.json` теперь содержит explicit metadata-слой `APP_DATA.corpus`. Первый источник — текущая книга `zaliznyak-aaz-index`; будущие маршруты могут использовать aliases вида `#v4/books/<book_id>/names/list`, которые канонизируются обратно в существующий route state. Runtime сохраняет compatible default только как fallback для старых сборок без `corpus`.

В `APP_DATA.corpus.source_types` зарезервирован тип `video_catalog`: он предназначен для каталога примерно 200 видео Зализняка с `timecodes`, стенограммами, цитированием и media-ссылками. Это пока metadata-задел, без миграции `schema_version` и без изменения структуры основных сущностей.

Глобальный поиск уже имеет переключатель области: `книга` ищет в текущем источнике, `корпус` оставляет тот же UI-контракт для будущих книг и видеокаталога. Результаты показывают source context, чтобы при расширении корпуса пользователь видел, из какой книги или коллекции пришла строка.

В режиме `корпус` dropdown поиска группирует результаты по источнику и показывает понятное empty state, если совпадений нет. Списки сущностей, карточки, KWIC-строки, Markdown exports карточек/разделов/scholar/site, BibTeX exports, `page_trends` exports и VIZ header также показывают текущую книгу/source context, а exports дополнительно включают `book_id`, чтобы будущая multi-book навигация не требовала нового UI-паттерна.

Скопированные share links добавляют `books=<book_id>` к canonical hash, а runtime читает этот параметр при открытии маршрута и выставляет активный источник, если книга известна корпусному registry.

Экран `#v4/corpus/sources` показывает текущие и запланированные источники корпуса: первую книгу, типы источников и будущий видеокаталог.
...
python scripts/content_report.py --write-manual-audit tests/index-audit-queue.json
...
Текущая редакторская очередь: 36 suspicious heads, 23 sort inversions, 0 duplicate-head groups. Из manual audit найдено 7 из 11 терминов; без точного `head` сейчас остаются `выругать`, `суоми`, `счастлив`, `счастье`.

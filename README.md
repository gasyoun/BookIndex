...
*   **KWIC (Key Word in Context)**: Конкорданс по книге (лексика, глоссарий) **и по корпусу расшифровок 27 видеолекций** — каждое совпадение ведёт на минуту в видео. Индекс лекций строится `npm run kwic:build` (ленивая подгрузка `data/lectures_kwic.json`).

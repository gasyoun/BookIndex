...
В репозитории этот набор закреплен как `tests/e2e/navigation-architecture.spec.js`; быстрый запуск: `npm run check:navigation`. Gemini Flash может предлагать расширения матрицы, но не должен удалять существующие маршруты из контракта без отдельного решения.

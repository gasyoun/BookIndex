var BookIndex = (function(exports) {
	...
	var state_exports = /* @__PURE__ */ __exportAll({
		...
		APP_DATA: () => APP_DATA$1,
		APP_DATA_GLOBAL_FALLBACK_KEY: () => APP_DATA_GLOBAL_FALLBACK_KEY,
		APP_DATA_MODULES_BASE_URL: () => APP_DATA_MODULES_BASE_URL,
		APP_DATA_SCHEMA_CURRENT: () => 2,
		APP_DATA_SCRIPT_TAG_ID: () => APP_DATA_SCRIPT_TAG_ID,
		...
		ITEM_HASH_HEAD_BY_SLUG: () => ITEM_HASH_HEAD_BY_SLUG,
		ITEM_HASH_SLUG_BY_HEAD: () => ITEM_HASH_SLUG_BY_HEAD,
		ITEM_INDEX_EXACT: () => ITEM_INDEX_EXACT,
		ITEM_INDEX_NORMALIZED: () => ITEM_INDEX_NORMALIZED,
		...
		NORMALIZE_CACHE_LIMIT: () => NORMALIZE_CACHE_LIMIT,
		...
		normalizeHeadCache: () => normalizeHeadCache,
		...
		suppressHashSync: () => suppressHashSync,
	...
	var APP_DATA_MODULES_BASE_URL = "./data/modules/";
	var KWIC_MAX_ROWS$1 = 1200;
	var APP_BUILD_ID$1 = "__APP_BUILD_ID__";
	var NORMALIZE_CACHE_LIMIT = 5e3;
	var APP_DATA$1 = null;
	var LABELS$1 = null;
	var COLORS$1 = null;
	...
	var ITEM_INDEX_EXACT = /* @__PURE__ */ new Map();
	var ITEM_INDEX_NORMALIZED = /* @__PURE__ */ new Map();
	var CHAPTER_ITEM_INDEX$1 = /* @__PURE__ */ new Map();
	var ITEM_HASH_SLUG_BY_HEAD = /* @__PURE__ */ new Map();
	var ITEM_HASH_HEAD_BY_SLUG = /* @__PURE__ */ new Map();
	...
	var normalizeHeadCache = /* @__PURE__ */ new Map();
	...
	var utils_exports = /* @__PURE__ */ __exportAll({
		...
		normalizeHashSlug: () => normalizeHashSlug,
	...
	var CYRILLIC_TO_LATIN_MAP = Object.freeze({
		"а": "a",
		"б": "b",
		"в": "v",
		"г": "g",
	...
	function normalizeHeadForMatch$1(value) {
		...
		rememberBoundedCacheValue$1(normalizeHeadCache, raw, s, NORMALIZE_CACHE_LIMIT, { clearWhenFull: true });
		return s;
	}
	...
	function encodeItemHeadForHash$1(type, head) {
		...
		return normalizeHashSlug(resolved) || resolved;
	}
	...
	*/
	...
	var data_exports = /* @__PURE__ */ __exportAll({
		DESCRIPTION_FIELDS_WITH_NORMALIZED_YO: () => DESCRIPTION_FIELDS_WITH_NORMALIZED_YO,
		LECTURE_WHY_READ_BROTHER_BRAT: () => LECTURE_WHY_READ_BROTHER_BRAT,
		TAB_LABELS: () => TAB_LABELS$1,
		appDataModuleUrl: () => appDataModuleUrl,
		applyActiveBookFromQuery: () => applyActiveBookFromQuery,
		applyDescriptionEditorialConventions: () => applyDescriptionEditorialConventions,
		buildAllItems: () => buildAllItems,
		buildDataIndexes: () => buildDataIndexes,
		buildDefaultCorpusRegistry: () => buildDefaultCorpusRegistry,
		fetchAppDataModule: () => fetchAppDataModule,
		...
		getEmbeddedAppDataText: () => getEmbeddedAppDataText,
		getFirstContextQuote: () => getFirstContextQuote$1,
		hydrateAppData: () => hydrateAppData,
		indexItems: () => indexItems,
		initEntityTypes: () => initEntityTypes,
		isAppDataModuleManifest: () => isAppDataModuleManifest,
		loadAppData: () => loadAppData,
		loadAppDataFromModules: () => loadAppDataFromModules,
		migrateAppDataSchema: () => migrateAppDataSchema,
		normalizeAppData: () => normalizeAppData,
		normalizeContextSnippet: () => normalizeContextSnippet,
		normalizeCorpusRegistry: () => normalizeCorpusRegistry,
		normalizeDescriptionYoInNode: () => normalizeDescriptionYoInNode,
		normalizeDescriptionYoText: () => normalizeDescriptionYoText,
		normalizeEditorialFlags: () => normalizeEditorialFlags,
		normalizeItemContexts: () => normalizeItemContexts,
		normalizeItemSources: () => normalizeItemSources,
		parseAppData: () => parseAppData
	});
	var DESCRIPTION_FIELDS_WITH_NORMALIZED_YO = /* @__PURE__ */ new Set([
		"desc",
		"about",
		"why",
		"why_read",
	...
	var LECTURE_WHY_READ_BROTHER_BRAT = "Чтобы понять, почему «brother» и «брат» — родственники, а не дети «санскрита», и как это узнают ученые.";
	function getEmbeddedAppDataText() {
		...
		const fallback = typeof globalThis !== "undefined" && typeof globalThis["__APP_DATA_STRING__"] === "string" ? globalThis[APP_DATA_GLOBAL_FALLBACK_KEY] : "";
	...
	function parseAppData() {
		...
		if (isAppDataModuleManifest(data)) throw new Error("Embedded app data contains a module manifest; use loadAppData() instead");
	...
	async function loadAppData() {
		...
		return hydrateAppData(isAppDataModuleManifest(parsed) ? await loadAppDataFromModules(parsed) : parsed);
	}
	...
	function getCorpusRegistry$1() {
		if (!APP_DATA$1 || !APP_DATA$1.corpus || typeof APP_DATA$1.corpus !== "object") return buildDefaultCorpusRegistry();
	...
	function applyDescriptionEditorialConventions() {
		normalizeDescriptionYoInNode(APP_DATA$1);
		const lectures = Array.isArray(APP_DATA$1?.lectures) ? APP_DATA$1.lectures : [];
		if (lectures[2] && typeof lectures[2].why_read === "string" && lectures[2].why_read.includes("brother") && lectures[2].why_read.includes("брат")) lectures[2].why_read = LECTURE_WHY_READ_BROTHER_BRAT;
	}
	function initEntityTypes() {
		...
		ENTITY_TYPES$1.all.items = buildAllItems();
		for (const key of Object.keys(ENTITY_TYPES$1)) indexItems(ENTITY_TYPES$1[key].items);
		buildDataIndexes();
	...
	function buildHashFromState$1() {
		...
		if (currentEntity$1 === "materials" && currentTab$1 === "video" && currentVideoId) parts.push(String(currentVideoId));
	...
	function pushHistoryState() {
		...
		if (sameViewState$1(historyStack.length ? historyStack[historyStack.length - 1] : null, snap)) return;
	...
	function applyHash$1(hash) {
		...
		if (state.selectedItem) displayTitle = `${state.selectedItem} (детали)`;
			...
			const hit = (APP_DATA$1 && Array.isArray(APP_DATA$1.video_catalog) ? APP_DATA$1.video_catalog : []).find((v) => v && v.id === state.currentVideoId);
	...
	//#endregion
	//#region src/runtime/legacy.js
	var REPLACEMENT_CHAR = String.fromCharCode(65533);
	...
	function buildEntityIssueUrl(type, head) {
		...
		return `https://github.com/gasyoun/BookIndex/issues/new?${new URLSearchParams({
	...
	function ensureVizStateLoaded() {
		...
		return (stateReady ? Promise.resolve() : loadVizScriptOnce(VIZ_STATE_SCRIPT_PATH)).then(() => {
	...
	function normalizeCommandPaletteText(value) {
		return normalizeSearchText(String(value === null || value === void 0 ? "" : value).toLowerCase());
	}
	...
	function runCommandPaletteEntry(entry) {
		...
		if (action.indexOf("density:") === 0) {
			applyDensityMode(action.slice(8));
			return true;
		}
		if (action === "back") {
			goBackInApp();
	...
	function onCommandPaletteKeydown(e) {
		...
		if (e.key === "Enter") {
			flushCommandPaletteQuery();
			runCommandPaletteEntry(commandPaletteEntries[commandPaletteActiveIndex < 0 ? 0 : commandPaletteActiveIndex]);
			e.preventDefault();
		}
	}
	...
	var QUALITY_SORT_ENTITY_KEYS = /* @__PURE__ */ new Set([
		"names",
		"toponyms",
		"ethnonyms",
		"languages",
	...
	function getQualityPageVerificationDetails(item) {
		...
		const evidenceSet = /* @__PURE__ */ new Set([...occurrencePages, ...contextPages]);
		...
		return {
			count: (/* @__PURE__ */ new Set([...missingFromPageList, ...missingFromEvidence])).size,
			pageList,
			occurrencePages,
			contextPages,
			missingFromPageList,
  ...
  `;
	...
	function videoBacklinkKey(type, head) {
		return (type === "subject_index" ? "subject" : String(type || "")) + "\0" + String(head || "");
	}
	...
	function getVideoBacklinkIndex() {
		...
		for (const v of catalog) {
			...
			for (const rel of rels) {
				...
				idx.get(key).push({
					v,
					t: Number.isFinite(rel.t) ? rel.t : null
				});
			}
		}
		for (const list of idx.values()) list.sort((a, b) => {
	...
	function getVideoRelatedChapters(video, limit = 8) {
		...
		for (let i = 0; i < chapters.length; i++) {
			const found = getChapterRelatedVideos(i, 9999).find((e) => e && e.v && (e.v.id === video.id || e.v.url === video.url));
			if (found) out.push({
				chapter: chapters[i],
				index: i,
				score: found.score
    ...
    <div class="video-detail-intro">В каталоге нет записи с id «${escapeHtml(safeId || videoId || "")}».</div>
		...
		const metaParts = [formatVideoDate(video.date), dur ? `⏱ ${dur}` : ""].filter(Boolean);
		...
		let chaptersHtml = "";
		if (chapters.length) for (const c of chapters) {
		...
		else chaptersHtml = `<div class="video-detail-empty">Тематическое пересечение с главами книги не найдено (по связанным сущностям).</div>`;
	...
	function getVideoModalFocusable() {
		const root = document.getElementById("video-player-modal");
		if (!root || typeof root.querySelectorAll !== "function") return [];
		return Array.from(root.querySelectorAll("a[href], button:not([disabled]), iframe, [tabindex]:not([tabindex='-1'])")).filter((el) => el instanceof HTMLElement);
	}
	...
	function getChapterRelatedVideos(chapterIdx, limit = 6) {
		...
		const ch = (Array.isArray(APP_DATA.chapters) ? APP_DATA.chapters : [])[chapterIdx];
		const catalog = getDedupedVideoCatalog();
		...
		for (const t of types) for (const it of APP_DATA[t] || []) for (const p of it.page_list || []) {
		...
		for (const v of catalog) {
			...
			const seen = /* @__PURE__ */ new Set();
			let score = 0;
			for (const rel of v.related_entities || []) if (rel && heads.has(rel.head) && !seen.has(rel.head)) {
				seen.add(rel.head);
				score += 1;
			}
  ...
  `;
      ...
      <strong>Примечание о подсчёте:</strong> ${escapeHtml(it.moderator_note)}
		...
		if (videoBacklinks.length) {
			const shown = videoBacklinks.slice(0, 8);
			...
			for (const entry of shown) {
				...
				const meta = hasT ? `<span class="card-video-dur card-video-at" title="упоминание на ${escapeHtml(formatVideoDuration(entry.t))}">▸ ${escapeHtml(formatVideoDuration(entry.t))}</span>` : formatVideoDuration(v.duration) ? `<span class="card-video-dur">${escapeHtml(formatVideoDuration(v.duration))}</span>` : "";
		...
		let lodHtml = "";
		if (eType === "languages") {
			const info = LANGUAGE_LOD_MAP[String(it.head || "").toLowerCase().trim()];
			if (info) {
			...
			const cleanTerm = String(it.head || "").normalize("NFD").replace(/[\u0300-\u036f*?]/g, "").trim();
			if (cleanTerm) {
    ...
    <div class="legend" id="timeline-legend"></div></div></div>`;
		...
		if (isNarrow) {
			...
			for (let i = 0; i < withEpoch.length; i++) {
				...
				svg += `<text x="92" y="${y + 4}" fill="#888" font-size="10" text-anchor="end">${epochLabel}</text>`;
        ...
        <text x="110" y="${y + 4}" fill="#1a1a1a" font-size="12">${escapeHtml(n.head)}</text>
      </g>`;
			}
			svg += "</svg>";
			tl.innerHTML = svg;
			...
			for (let i = 0; i < ticks.length; i++) {
				...
				svg += `<line x1="${x}" y1="34" x2="${x}" y2="46" stroke="#8a7050" stroke-width="2"/>`;
				svg += `<line x1="${x}" y1="46" x2="${x}" y2="${H - 5}" stroke="#f0e8d8" stroke-width="1" stroke-dasharray="2 3"/>`;
				let label;
				if (t < 0) label = -t + " до н.э.";
				else if (t === 0) label = "0";
				else label = String(t);
				svg += `<text x="${x}" y="28" fill="#5a3818" font-size="11" text-anchor="middle">${label}</text>`;
			}
			for (const p of placed) {
				const cy = 58 + p.level * rowH;
    ...
    <div id="heatmap"></div></div></div>`;
		...
		for (let i = 0; i < top.length; i++) {
			...
			svg += `<text x="214" y="${y + 4}" fill="#1a1a1a" font-size="10" text-anchor="end">${escapeHtml(label)}</text>`;
			const color = currentEntity === "names" ? safeColor(COLORS[it.subcategory], "#888") : "#5a3818";
			for (const p of it.page_list || []) {
				const x = labelW + (p - 1) * cellW;
				svg += `<rect x="${x}" y="${y - 4}" width="${Math.max(2.5, cellW)}" height="10" fill="${color}" opacity="0.85"><title>${escapeHtml(it.head)} · стр. ${p}</title></rect>`;
			}
		}
		svg += "</svg>";
		hm.innerHTML = svg;
	...
	function buildHomeIndexTaskEntries() {
		...
		return (section && Array.isArray(section.items) ? section.items : []).filter((item) => item && item.entity && ENTITY_TYPES[item.entity]).map((item) => {
			const conf = ENTITY_TYPES[item.entity];
	...
	function renderHomePanel(container) {
		...
		const quoteTextClass = compactHome ? "home-featured-quote-clamp" : "";
		const routeGridClass = compactHome || isDesktop ? "home-routes-grid home-routes-grid-compact" : "home-routes-grid";
		const indexOptionsHtml = buildHomeIndexTaskEntries().map((entry) => `<option value="${escapeHtml(entry.entity)}">${escapeHtml(entry.label)}${entry.count ? ` (${entry.count})` : ""}</option>`).join("");
    ...
    <div id="vg-list" class="video-gallery-list vg-list-dense"></div>
		...
		const render = () => {
			...
			if (q) vids = vids.filter((v) => String(v.title || "").toLowerCase().includes(q) || (v.related_entities || []).some((r) => String(r && r.head || "").toLowerCase().includes(q)));
			vids.sort((a, b) => {
			...
			for (const v of vids) {
				...
				const dur = formatVideoDuration(v.duration);
				meta.textContent = [formatVideoDate(v.date) || "дата неизвестна", dur ? `⏱ ${dur}` : ""].filter(Boolean).join(" · ");
				card.appendChild(meta);
				if (v.url) {
	...
	var GLOSSARY_TOKEN_STOPWORDS = /* @__PURE__ */ new Set([
		"это",
		"этот",
		"эта",
		"эти",
	...
	function loadLecturesKwic() {
		...
		lecturesKwicLoadPromise = fetch(url, { cache: "default" }).then((r) => {
			if (!r.ok) throw new Error(`HTTP ${r.status}`);
			return r.json();
		}).then((d) => {
			LECTURES_KWIC_CACHE = d && Array.isArray(d.segments) ? d : {
				videos: [],
				segments: []
			};
			return LECTURES_KWIC_CACHE;
		}).catch((e) => {
			lecturesKwicLoadPromise = null;
			throw e;
		});
	...
	function collectLecturesKwicRows(query) {
		...
		for (const seg of cache.segments) {
			...
			while (m !== null) {
				...
				if (pat.lastIndex === idx) pat.lastIndex++;
				m = pat.exec(lower);
			}
		}
		return rows;
    ...
    <div class="kwic-inner">
      ...
      <div id="kwic-results" class="kwic-results"></div>
		...
		const renderRows = () => {
			...
			metaEl.textContent = currentKwicSource === "lectures" ? `Найдено ${rows.length} контекстов в ${lecturesCount} лекциях, источник: лекции.${truncText}` : `Найдено ${rows.length} контекстов (${terms.size} терминов), источник: ${sourceLabel}.${truncText}`;
	...
	function renderPhoneticLawsPanel(container) {
		...
		const simData = [
			{
				...
				branches: {
					russian: [
						{
							state: "*gordъ",
							law: "Исходная праславянская форма",
							desc: "Праславянское начальное состояние слова до разделения."
						},
						{
							state: "*gorodъ",
							law: "Восточнославянское полногласие",
							desc: "Группа -or- переходит в полногласное сочетание -оро-."
						},
						{
							state: "город",
							law: "Падение слабых редуцированных",
							desc: "Конечный слабый гласный -ъ полностью отпадает."
						}
					],
					polish: [
						{
						...
						{
							state: "*grodъ",
							law: "Лехитское неполногласие",
							desc: "Группа -or- переходит в неполногласное сочетание -ro-."
						},
						{
							state: "gród",
							law: "Падение редуцированных и сужение гласного",
							desc: "Утрата конечного -ъ и сужение гласного о -> ó перед звонким согласным."
						}
					],
					czech: [
						{
						...
						{
							state: "*gradъ",
							law: "Чешско-словацкое неполногласие",
							desc: "Группа -or- переходит в неполногласное сочетание -ra-."
						},
						{
							state: "*hradъ",
							law: "Спирантизация согласного g -> h",
							desc: "Древний взрывной согласный g переходит в придыхательный h."
						},
						{
							state: "hrad",
							law: "Падение редуцированных",
							desc: "Конечный редуцированный гласный -ъ отпадает."
						}
					]
				}
			},
					...
					russian: [
						{
							state: "*melko",
							law: "Исходная праславянская форма",
							desc: "Праславянское начальное состояние слова."
						},
						{
							state: "*moloko",
							law: "Восточнославянское полногласие",
							desc: "Группа -el- переходит в полногласное сочетание -оло-."
						},
						{
							state: "молоко",
							law: "Закрепление современной формы",
							desc: "Формирование современного русского литературного произношения."
						}
					],
					polish: [
						{
						...
						},
						{
							state: "*mleko",
							law: "Лехитское неполногласие",
							desc: "Группа -el- переходит в неполногласное сочетание -le-."
						},
						{
							state: "mleko",
							law: "Закрепление современной формы",
							desc: "Формирование современного польского литературного слова."
						}
					],
					czech: [
						{
						...
						{
							state: "*mleko",
							law: "Чешское неполногласие",
							desc: "Группа -el- переходит в неполногласное сочетание -le-."
						},
						{
							state: "mléko",
							law: "Долгота гласного",
							desc: "Вокализация с удлинением гласного звука: e -> é."
						}
					]
				}
			},
			{
				...
				branches: {
					russian: [
						...
						{
							state: "дорогой",
							law: "Добавление флексии прилагательного",
							desc: "Падение конечного -ъ и оформление флексии мужского рода -ой."
						}
					],
					polish: [
						{
						...
						{
							state: "drogi",
							law: "Утрата редуцированного и флексия прилагательного",
							desc: "Конечный -ъ отпадает, добавляется окончание именительного падежа -i."
						}
					],
					czech: [
						{
						...
						{
							state: "*drahъ",
							law: "Чешское неполногласие и спирантизация",
							desc: "Группа -or- переходит в -ra-, согласный g переходит в h."
						},
						{
							state: "drahý",
							law: "Падение редуцированного и флексия прилагательного",
							desc: "Утрата конечного -ъ и присоединение окончательной флексии -ý."
						}
					]
				}
			}
		...
		const maxSteps = Math.max(currentRoot.branches.russian.length, currentRoot.branches.polish.length, currentRoot.branches.czech.length);
		...
		for (let i = 0; i < simData.length; i++) simHtml += `<option value="${i}"${i === currentSimRootIndex ? " selected" : ""}>${escapeHtml(simData[i].root)}</option>`;
		simHtml += `
				</select>
		...
		const renderBranch = (title, stepsList) => {
			...
			return `
				<div class="sim-branch-col ${currentSimStep === activeIndex ? "highlighted" : ""}">
	...
	function renderScholarPanel(container) {
		...
		function hydrateToOldRussian(word) {
			...
			const yatRoots = [
				{
					...
					replace: "хлѣб",
					name: "Корень *хлѣб-",
					desc: "Общеславянский ять в корне слова."
				},
				{
					regex: /вер/g,
					replace: "вѣр",
					name: "Корень *вѣр-",
					desc: "Общеславянский ять (вера, верный)."
				},
				{
					regex: /мест/g,
					replace: "мѣст",
					name: "Корень *мѣст-",
					desc: "Общеславянский ять (место, местный)."
				},
				{
					regex: /снег/g,
					replace: "снѣг",
					name: "Корень *снѣг-",
					desc: "Общеславянский ять (снег, снеговой)."
				},
				{
					regex: /бел/g,
					replace: "бѣл",
					name: "Корень *бѣл-",
					desc: "Общеславянский ять (белый, белок)."
				},
				{
					regex: /веч/g,
					replace: "вѣч",
					name: "Корень *вѣч-",
					desc: "Общеславянский ять (вече, вечный)."
				},
				{
					regex: /дед/g,
					replace: "дѣд",
					name: "Корень *дѣд-",
					desc: "Общеславянский ять (дед, дедушка)."
				},
				{
					regex: /грех/g,
					replace: "грѣх",
					name: "Корень *грѣх-",
					desc: "Общеславянский ять (грех, грешный)."
				},
				{
					regex: /звер/g,
					replace: "звѣр",
					name: "Корень *звѣр-",
					desc: "Общеславянский ять (зверь, звериный)."
				},
				{
					regex: /лес/g,
					replace: "лѣс",
					name: "Корень *лѣс-",
					desc: "Общеславянский ять (лес, лесной)."
				},
				{
					regex: /сен/g,
					replace: "сѣн",
					name: "Корень *сѣн-",
					desc: "Общеславянский ять (сено, сенокос)."
				},
				{
					regex: /цвет/g,
					replace: "цвѣт",
					name: "Корень *цвѣт-",
					desc: "Общеславянский ять (цвет, цветок)."
				},
				{
					regex: /тел/g,
					replace: "тѣл",
					name: "Корень *тѣл-",
					desc: "Общеславянский ять (тело, телесный)."
				},
				{
					regex: /дел/g,
					replace: "дѣл",
					name: "Корень *дѣл-",
					desc: "Общеславянский ять (дело, делать)."
				},
				{
					regex: /вед/g,
					replace: "вѣд",
					name: "Корень *вѣд-",
					desc: "Общеславянский ять (ведать, известный)."
				}
			];
			let triggeredYat = false;
			for (const r of yatRoots) if (r.regex.test(current)) {
			...
			for (const r of bigYusRoots) if (r.regex.test(current)) {
				current = current.replace(r.regex, r.replace);
				if (!triggeredBigYus) {
					rules.push({
						title: "Большой юс (ѫ)",
			...
			for (const r of smallYusRoots) if (r.regex.test(current)) {
				current = current.replace(r.regex, r.replace);
		...
		function highlightStressedVowel(word) {
			if (!word || word === "—") return "—";
			return String(word).replace(/([аеиоуыэюяё][\u0301]?|[âêîôûàèìòùỳ])/gi, (match) => {
				if (match.includes("́") || match.toLowerCase() === "ё" || /[âêîôûàèìòùỳ]/i.test(match)) return `<span class="accent-syllable-stressed">${match}</span>`;
				return match;
			});
		}
					...
					},
					{
						...
						forms: {
							sg: {
								nom: "го́родъ",
								gen: "города́",
								gen_preposition: "о́коло города",
								dat: "го́роду",
								acc: "го́родъ",
								acc_preposition: "на́ городъ",
								ins: "го́родомь",
								loc: "го́роде"
							},
							pl: {
    ...
    <div id="accent-compare-box"></div>
		...
		for (let i = 0; i < ACCENT_RECON_DATA.length; i++) reconHtml += `<option value="${i}"${i === currentAccentReconRootIndex ? " selected" : ""}>${escapeHtml(ACCENT_RECON_DATA[i].stem)} (${escapeHtml(ACCENT_RECON_DATA[i].meaning)})</option>`;
		reconHtml += `
					</select>

					<label class="sim-label">Историческая стадия:</label>
					<div class="accent-recon-stages">`;
		for (let i = 0; i < currentReconRoot.stages.length; i++) reconHtml += `<button type="button" class="accent-recon-stage-btn${i === currentAccentReconStageIndex ? " active" : ""}" data-stage-idx="${i}">${escapeHtml(currentReconRoot.stages[i].name)}</button>`;
		reconHtml += `
					</div>
				</div>
		...
		for (const [cKey, cName] of [
			...
			let extraSg = "";
			if (currentReconStage.forms.sg[cKey + "_preposition"]) extraSg = `<br><span class="scholar-note">с предлогом: ${highlightStressedVowel(currentReconStage.forms.sg[cKey + "_preposition"])}</span>`;
			reconHtml += `
								<tr>
		...
		if (hydrated.rules.length === 0) orthoHtml += `<div class="scholar-table-empty">Слово сохранено в исходной графике (за исключением общих правил). Попробуйте ввести примеры из списка слева.</div>`;
		else for (const rule of hydrated.rules) orthoHtml += `
    ...
    <label class="scholar-filter-label">Фонетический закон
      ...
      <span class="scholar-muted-meta">${escapeHtml((item.page_list || []).length)} стр.</span>
		...
		if (reconSelect) reconSelect.onchange = (e) => {
			currentAccentReconRootIndex = parseInt(e.target.value, 10) || 0;
			if (currentAccentReconStageIndex >= ACCENT_RECON_DATA[currentAccentReconRootIndex].stages.length) currentAccentReconStageIndex = 0;
		...
		if (orthoInput) {
			orthoInput.oninput = (e) => {
				...
				if (list) {
					if (hyd.rules.length === 0) list.innerHTML = `<div class="scholar-table-empty">Слово сохранено в исходной графике (за исключением общих правил). Попробуйте ввести примеры из списка слева.</div>`;
					else list.innerHTML = hyd.rules.map((rule) => `
							<div class="ortho-rule-alert">
								<div class="ortho-rule-title">${escapeHtml(rule.title)}</div>
								<div>${escapeHtml(rule.desc)}</div>
							</div>
    ...
    <div id="lang-tree"></div></div></div>`;
		...
		for (const fam of tree) {
			...
			for (const grp of fam.children) {
				addPath(`M 200 ${fam.midY} C 200 ${fam.midY}, 200 ${grp.midY}, ${col2} ${grp.midY}`, {
					fill: "none",
					stroke: famColor,
					"stroke-width": 1.5,
					opacity: .6
				...
				for (const lang of grp.children) {
					...
					if (!p) continue;
					addPath(`M 460 ${grp.midY} C 460 ${grp.midY}, 460 ${p.y}, ${col3} ${p.y}`, {
						fill: "none",
						stroke: famColor,
						"stroke-width": 1,
						opacity: .4
					...
					circle.setAttribute("cx", String(474));
  ...
  `;
	...
	var LANGUAGE_LOD_MAP = {
		...
		"праславянский": {
			...
			wals: "psl",
			glottolog: "prot1234",
			customWals: "https://wals.info/languoid/lect/wals_code_psl"
		},
		"старославянский": {
			iso: "chu",
			wals: "ocs",
	...
	*/
	var BOOT_LOG_STORAGE_KEY = "Zalizniakiada.debug.boot";

...
function buildHomeHowToGuideHtml() {
  ...
  return `<details id="home-howto-details" class="home-howto" open>
    ...
    <div class="home-howto-grid">
      ...
      <p class="home-howto-p">Раздел <a href="${escapeHtml(namesListHash)}" style="${linkStyle}">«Имена»</a> — это указатель ученых, авторов и исторических фигур. Например, можно открыть <a href="${escapeHtml(avanesovItemHash)}" style="${linkStyle}">Аванесов Р. И.</a> или найти <a href="${escapeHtml(avvakumHash)}" style="${linkStyle}">Аввакум [Петров]</a>.</p>
      ...
      <p class="home-howto-p">Раздел <a href="${escapeHtml(ethnonymsListHash)}" style="${linkStyle}">«Этнонимы»</a> собирает названия народов и этнических групп. Удобно проверять страницы, где народ обсуждается как носитель языка, традиции или контактной зоны.</p>

      <h4 class="home-howto-h4">Языки</h4>
      <p class="home-howto-p">Раздел <a href="${escapeHtml(languagesListHash)}" style="${linkStyle}">«Языки»</a> — указатель языков и языковых групп. Пример: <a href="${escapeHtml(austraItemHash)}" style="${linkStyle}">австралийские</a> в списке и точка в <a href="${escapeHtml(languagesMapHash)}" style="${linkStyle}">карте языков</a>.</p>

      <h4 class="home-howto-h4">Лексика</h4>
      <p class="home-howto-p">Раздел <a href="${escapeHtml(lexiconListHash)}" style="${linkStyle}">«Лексика»</a> — словарный указатель к книге. <a href="${escapeHtml(lexiconReverseHash)}" style="${linkStyle}">«Лексика (обратная)»</a> полезна, если хотите искать не по началу слова, а по окончанию, корню или модели.</p>

      <h4 class="home-howto-h4">Реконструкции</h4>
      <p class="home-howto-p"><a href="${escapeHtml(reconstructionsHash)}" style="${linkStyle}">«Реконструкции»</a> — небольшой технический раздел для восстановленных форм и специальных обозначений.</p>

      <h4 class="home-howto-h4">Предметный указатель</h4>
      <p class="home-howto-p"><a href="${escapeHtml(subjectListHash)}" style="${linkStyle}">«Предметный»</a> отвечает на вопрос не «где встречается слово», а «где в книге разбирается явление»: <a href="${escapeHtml(akanyeSubjectHash)}" style="${linkStyle}">аканье</a>, <a href="${escapeHtml(birchSubjectHash)}" style="${linkStyle}">берестяные грамоты</a>, <a href="${escapeHtml(articleSubjectHash)}" style="${linkStyle}">артикль</a>, <a href="${escapeHtml(udarenieSubjectHash)}" style="${linkStyle}">ударение</a>.</p>
      ...
      <p class="home-howto-p"><strong>Глобальный поиск</strong> в шапке страницы: вводите имя, язык, топоним, термин или лекцию. Примеры: <a href="${escapeHtml(angliaHash)}" style="${linkStyle}">Англия</a>, <a href="${escapeHtml(udarenieAllHash)}" style="${linkStyle}">ударение</a>, <a href="${escapeHtml(globalLekciiHash)}" style="${linkStyle}">лекция</a>.</p>
      <p class="home-howto-p"><strong>Локальный поиск</strong> внутри раздела: фильтрует только текущий список. Для контекстов по фрагментам используйте <a href="${escapeHtml(kwicHash)}" style="${linkStyle}">KWIC</a> и, например, запрос <a href="${escapeHtml(sanskritHash)}" style="${linkStyle}">санскрит</a>.</p>

      <h3 class="home-howto-h3">Карта и примеры</h3>
      <p class="home-howto-p">Карта работает в разделах <a href="${escapeHtml(toponymsMapHash)}" style="${linkStyle}">«Топонимы»</a>, <a href="${escapeHtml(ethnonymsMapHash)}" style="${linkStyle}">«Этнонимы»</a> и <a href="${escapeHtml(languagesMapHash)}" style="${linkStyle}">«Языки»</a>.</p>
      <ul class="home-howto-list">

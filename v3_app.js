...
function renderGlobalSearchResults(matches, query = '') {
  ...
  box.querySelectorAll('.header-search-item').forEach(row => {
    row.onclick = () => {
      const m = matches[parseInt(row.dataset.idx || '0', 10)];
      if (!m) return;
      if (m.type === 'lecture') openLecturePage(m.lectureIndex || 0);
      else if (m.type === 'glossary') openGlossaryTerm(m.head || '');
      else navigateToItem(m.type, m.head);
...
function renderList() {
  ...
  list.innerHTML = '';
  const reverseColumns = currentEntity === 'lexicon_reverse'
    ? (typeof window !== 'undefined' && window.innerWidth >= 1500 ? 4
      : (typeof window !== 'undefined' && window.innerWidth >= 1200 ? 3
      : (typeof window !== 'undefined' && window.innerWidth >= 900 ? 2 : 1)))
    : 1;
  const useVirtual = filtered.length > 1000 && ['all', 'lexicon', 'lexicon_reverse'].includes(currentEntity);
  if (reverseColumns > 1 && !useVirtual) {
    list.style.columnCount = String(reverseColumns);
  ...
  if (useVirtual) {
    ...
    renderListProgressive(list, filtered, currentEntity, reverseColumns);
  } else {
  ...
  if (truncated) {
    ...
    if (reverseColumns > 1) more.style.columnSpan = 'all';
...
function renderCardInRight() {
  ...
  if (eType === 'names' && APP_DATA.edges) {
    ...
    if (relatedEdges.length > 0) {
      ...
      for (const e of relatedEdges.slice(0, 10)) {
        ...
        html += `<div class="xlink" data-type="names" data-head="${escapeHtml(other)}" style="display:flex;justify-content:space-between;gap:8px;cursor:pointer;padding:2px 0;">
  ...
  for (const [tgtType, tgtMap] of Object.entries(cross)) {
    ...
    html += `<div class="xlink" data-type="${escapeHtml(tgtType)}" data-head="${escapeHtml(lnk.head)}" style="display:flex;justify-content:space-between;gap:8px;cursor:pointer;padding:2px 0;">
...
function renderHomePanel(container) {
  ...
  for (const r of routes) {
    ...
    for (const e of r.entities) {
      html += `<span class="route-link" data-type="${escapeHtml(e.type)}" data-head="${escapeHtml(e.head)}" style="display:inline-block;padding:2px 8px;background:#f0e8d8;border-radius:10px;margin:2px 2px 2px 0;cursor:pointer;color:#5a3818;text-decoration:underline dotted;">${escapeHtml(e.head)}</span>`;
    }
    html += '</div></div>';
  }
  html += '</div>';
  ...
  if (recentBox && recentItems.length) {
    ...
    for (const r of recentItems) {
      ...
      recentHtml += `<span class="home-recent-link" data-type="${escapeHtml(r.type)}" data-head="${escapeHtml(r.head)}" style="display:inline-block;padding:2px 8px;background:#f0e8d8;border-radius:10px;margin:2px 4px 2px 0;cursor:pointer;color:#5a3818;text-decoration:underline dotted;">${escapeHtml(r.head)} <span style="color:#777;">· ${escapeHtml(label)}</span></span>`;
    }
...
function renderLecturesPanel(container) {
  ...
  for (let i = 0; i < lectures.length; i++) {
    ...
    for (const t of l.terms) html += `<span class="lecture-term-chip" data-term="${escapeHtml(t.toLowerCase())}" style="display:inline-block;padding:2px 8px;background:#f0e8d8;border-radius:10px;color:#5a3818;text-decoration:underline dotted;">${escapeHtml(t)}</span>`;
    html += `</div>
...
function renderGlossaryPanel(container) {
  ...
  for (const g of glossary) {
    ...
    html += `<div class="glossary-entry" data-term="${escapeHtml(g.term.toLowerCase())}" style="background:#fff;border:1px solid #d4c8b0;border-radius:4px;padding:10px 14px;margin-bottom:8px;border-left:3px solid #8a7050;">
      <div style="font-size:14px;font-weight:bold;color:#5a3818;margin-bottom:4px;"><a href="${escapeHtml(safeUrl(termUrl))}" target="_blank" rel="noopener noreferrer" style="color:#5a3818;text-decoration:underline dotted;">${escapeHtml(g.term)}</a></div>
...
function renderScholarPanel(container) {
  ...
  for (const name of keyLinguists) {
    html += `<span class="scholar-link" data-type="names" data-head="${escapeHtml(name)}" style="display:inline-block;padding:4px 10px;background:#f0e8d8;border-radius:12px;margin:3px;cursor:pointer;color:#5a3818;text-decoration:underline dotted;font-size:12px;">${escapeHtml(name)}</span>`;
  }
  html += '</div>';
  ...
  for (const item of recon) {
    html += `<div class="scholar-link" data-type="lexicon_tech" data-head="${escapeHtml(item.head)}" style="display:flex;justify-content:space-between;gap:8px;padding:3px 0;cursor:pointer;">

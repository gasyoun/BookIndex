...
А. Кулыгин (передано МГ): значимое совпадение циклов — только арабские лекции; «Муми-тролль» — популярные лекции, ЛЛШ — научное изложение; таблицу соответствий не делать, ЛЛШ — библиография. МГ утвердил: одна оговорка про арабский, формулировка уровня дословно, онлайн-гид не трогать. Содержательная сверка дня (H4024 + хронология elementy/mathnet) не выброшена — ушла в служебную справку камера-ради: сцепка «февральская лекция → июльский доклад того же года» подтверждена для 2010 (ист. лингв. (продолж.) → о происхождении слов), 2012, 2013, 2016 годов; для 2011, 2014, 2015 годов близость по времени есть, темы чужие; у «Механизмов экспрессивности» (2011) соответствия нет ни в одну главу. Правка Полосы 5: сняты предложения-соответствия (496 + 74 знака), добавлены уровень, арабская оговорка и строка о каталоге. Полоса 4 не тронута (архив, не соответствия циклов).

| Показатель | До | После |
|---|---|---|
| Полоса 5 (знаки) | 2072 | **1888** (коридор 1800–2200) |
| Полоса 4 | 1878 | 1878 |
| Крест | 142/106/129 | без изменений |
| guide.html / гид | — | не регенерировались (`--check` байт-паритет ОК) |

**Model:** OxAlpha (`z-ai/glm-5.3-flash`).
**Handoff:** [H4028 (OxAlpha) — print: drop LLSH correspondence map](https://github.com/gasyoun/Uprava/blob/main/handoffs/H4028-OxAlpha_BookIndex_print-llsh-bibliography-only_03.09.26.md).
...
|---|---|---|---|
| 2007 береста | «Берестяные грамоты» | без изменений | `series` auto; куратор: ch06 112:14 |
| 2008 Велесова книга | «О жизни слов» | «История русского языка» | куратор ch06 34:50; ребра к ch10 нет; ch08 `rejected` |
| 2009 ударение | «Из русского ударения» | без изменений | `series` auto |
| 2010 происхождение слов | «О жизни слов» | «Историческая лингвистика (продолж.)» | куратор ch04 88:34 + ch08 82:31; к ch10 — только `auto` |
| 2011 экспрессивность | «О жизни слов» | — (без пары) | утверждённых связей нет (ch05 `rejected`, к ch10 — `auto`) |
| 2012 история русского языка | «История русского языка» | без изменений | куратор ch06 64:44 |
| 2013 арабский | «Арабский язык» | без изменений | `title_topics` auto; клетка ch07 пустая |
| 2014 внешняя сторона слова | «О жизни слов» | без изменений | куратор ch10 114:46 + ch06 112:04 |
| 2015 эпизод ударения | «Из русского ударения» | без изменений | `series` auto |
| 2016 ещё раз о жизни слов | «О жизни слов» | без изменений | по названию; ch08 `rejected` |
| 2017 живые механизмы | «Из русского ударения» | без изменений | куратор ch08 65:34 + ch06 66:18 |

Глав без своего доклада ЛЛШ — четыре: «Порядок слов», «Историческая лингвистика», «Древняя Индия», «Или и уже» (раньше строка называла три, без «Исторической лингвистики»). Правки: камера-реди (соседства + колонка «Основание» + печатная фраза Полосы 5), `LLSH_TOPIC_NEIGHBOR` в генераторе, сводный столбец «Соседний доклад ЛЛШ» теперь называет всех соседей главы. Крест 142/106/129 и `EXPECTED_APPROVED` не тронуты. Проверки: `build_reader_guide.py --check` (байт-паритет), census 1878/2072 (коридор 1800–2200), e2e `reader-guide.spec.js` 5/5.

**Model:** OxAlpha (`z-ai/glm-5.3-flash`).
**Handoff:** [H4024 (OxAlpha) — print: honest LLSH neighbor map + Полоса 5 prose before press](https://github.com/gasyoun/Uprava/blob/main/handoffs/H4024-OxAlpha_BookIndex_print-llsh-neighbor-fix_03.09.26.md).

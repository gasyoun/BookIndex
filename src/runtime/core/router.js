...
export function applyViewState(state) {
  ...
  if (!ENTITY_TYPES[targetEntity].tabs.includes(targetTab)) {
    targetTab = ENTITY_TYPES[targetEntity].tabs[0];
  }
  ...
  setCurrentKwicQuery(typeof state.currentKwicQuery === 'string'
  ...
  if (targetEntity === 'materials' && targetTab === 'glossary' && targetGlossaryTerm) {
    setPendingGlossaryQuery(targetGlossaryTerm);
  }
  if (targetEntity === 'scholar' && targetTab === 'scholar' && targetScholarAnchor) {
    setPendingScholarAnchor(targetScholarAnchor);
  }
  
  setOnlyDiscussed(!!state.onlyDiscussed);
  setOnlyQuestionCandidates(!!state.onlyQuestionCandidates);
  
  const filters = Array.isArray(state.activeFilters)
    ? new Set(state.activeFilters.filter(x => typeof x === 'string'))
    : new Set();
  setActiveFilters(filters);
  
  const targetSearchScope = Object.prototype.hasOwnProperty.call(state, 'globalSearchScope')
  ...
  if (typeof document !== 'undefined') {
    ...
    if (globalSearchInput) {
      globalSearchInput.value = typeof state.globalSearchQuery === 'string' ? state.globalSearchQuery : '';
    }
    const globalSearchScopeSelect = document.getElementById('global-search-scope');
    if (globalSearchScopeSelect && 'value' in globalSearchScopeSelect) {
      globalSearchScopeSelect.value = targetSearchScope;
    }
  }
...
export function applyHash(hash) {
  if (typeof window !== 'undefined' && typeof window.closeGlobalSearchResults === 'function') {
  ...
  const state = {
    ...
    currentLecture: 0,
    trendsRangeStart: 1,
    trendsRangeEnd: clampPageInBook(DEFAULT_TOTAL_PAGES),
    searchQuery: '',
    currentScholarAnchor: '',
  };
  setPendingGlossaryQuery('');
  ...
  if (entity === 'materials' && tab === 'lectures') {
    ...
    if (readingPage !== null) {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('Zalizniakiada.readingPage.v1', String(clampPageInBook(readingPage)));
      }
    }
  }
  if (entity === 'materials' && tab === 'glossary') {
    ...
    if (termValue) {
      setPendingGlossaryQuery(clampUiInput(termValue, MAX_LIST_QUERY_LENGTH).toLowerCase());
      setCurrentGlossaryTerm(clampUiInput(termValue, MAX_LIST_QUERY_LENGTH).toLowerCase());
    }
  }
  ...
  if (entity === 'scholar' && tab === 'viz') {
    ...
    if (moduleValue) {
      setCurrentVizModule(String(moduleValue || '').trim() || currentVizModule);
    }
  } else {
    setCurrentVizQueryString('');
  }
  ...
  // Dynamic SEO Metadata Integration (Phase 1 & 2)
  const routePath = `${state.currentEntity}/${state.currentTab}`;
  let displayTitle = '';
  if (state.selectedItem) {
    displayTitle = `${state.selectedItem} (детали)`;
  } else if (state.currentEntity === 'materials' && state.currentTab === 'lecture_pages') {
    const lectures = (APP_DATA && Array.isArray(APP_DATA.lectures)) ? APP_DATA.lectures : [];
    const l = lectures[state.currentLecture];
    const lectureTitle = state.currentLecture === 0 ? 'Предисловие' : `Лекция ${state.currentLecture}`;
    if (l && l.name) {
      displayTitle = `${lectureTitle}: ${l.name}`;
    } else {
      displayTitle = lectureTitle;
    }
  }
  updateSEOMetadata(routePath, displayTitle);

  // Dynamic Structured Data Integration (Phase 3)
  let schema = null;
  if (state.selectedItem) {
    ...
    if (l) {
      const lectureTitle = state.currentLecture === 0 ? 'Предисловие' : `Лекция ${state.currentLecture}`;
      const name = l.name ? `${lectureTitle}: ${l.name}` : lectureTitle;
